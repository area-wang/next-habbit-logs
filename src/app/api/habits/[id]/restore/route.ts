...
// POST /api/habits/[id]/restore - 恢复归档的习惯
export async function POST(
	...
	try {
		...
		await db
			.prepare(
				"INSERT INTO habit_archive_history (id, habit_id, user_id, action, timestamp, note) VALUES (?, ?, ?, 'restore', ?, NULL)"
			)
			.bind(historyId, id, user.id, now)
			.run();
