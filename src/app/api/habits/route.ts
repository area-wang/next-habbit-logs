...
export async function GET(req: NextRequest) {
	...
	const habitsRes = await getDb()
		.prepare(
			"SELECT id, title, description, frequency_type, frequency_n, active, created_at, updated_at FROM habits WHERE user_id = ? ORDER BY created_at DESC",
		)
		.bind(user.id)
		.all();
...
export async function POST(req: NextRequest) {
	...
	await getDb()
		.prepare(
			"INSERT INTO habits (id, user_id, title, description, frequency_type, frequency_n, active, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, 1, ?, ?)",
		)
		.bind(id, user.id, title, body?.description ? String(body.description) : null, frequencyType, frequencyN, now, now)
		.run();
